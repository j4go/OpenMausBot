# TypeScript Unit Testing Generator Reference

## Framework

- Prefer **Jest + ts-jest** or **Vitest**, following the project's existing
  setup.
...
Use `test.each` with explicit typing when needed.
...
test.each([
  ...
  'should return correct sum for various inputs',
  (input, expected) => {
    // Arrange
    const calc = new Calculator();
...
|---|---|---|---|---|
| `src/pages/LegacyPage.vue` component shell | Legacy framework mount env unavailable in this PR | Pure helper tests cover validation and payload mapping | @team | Before next UI refactor |
```
